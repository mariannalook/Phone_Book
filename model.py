...
class Edit_information:
    ...
    def sort(self):
        all_information.sort(key=lambda x: len(x["Surname"]))
    ...
    def edit_record(self):
        choose = input('''If you want edit for name - input 1
                        If you want edit for surname - input 2
    ...
    def tmp_edit(self, new_name, new_sername, new_phone, n):
        ...
        if new_sername != '':
            all_information[n]['Surname'] = new_sername
        if new_phone != '':
    ...
    def tmp_edit_2(self, choose, ed):
        ...
        if len(flag) == 1:
            ...
            Edit_information().tmp_edit(new_name, new_surname, new_phone, flag[0])
        if len(flag) > 1:
            ...
            new_surname = input('Input new surname')
            new_phone = input('Input new phone')
            Edit_information().tmp_edit(new_name, new_surname, new_phone, flag[ch])
    ...
    def see_contact(self):
        de = input('Input name,surname or number')
        for i in all_information:
            if de == i['Name']:
                print('Name: ', i['Name'], 'Surname: ', i['Surname'], 'Phone: ', i['Phone'])
            if de == i['Surname']:
